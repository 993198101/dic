...
// 字段类型变化，引起字段长度精度的校验
function choseColType(row) {
	switch (row.id) {
		case "20":// VARCHAR2
		case "12":// NVARCAHR2
			$('#colSize').numberspinner({required:true});
			$('#colSize').numberspinner('setValue', '20');
		break;
		case "24":// CHAR
		case "13":// ROW
		case "9": // NCHAR
		...
		break;
		default:
			$('#colSize').numberspinner({required:false});
	}
}
// 字段长度
function tableFileSize(size) {
	...
	case "1111":// NVARCAHR2
		if (size >= 1 && size <= 4000 && size.length >= 0) {
			return true;
		} else {
			return false;
		}
